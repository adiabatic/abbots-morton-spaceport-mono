...
# What one configuration in flight costs, and so the divisor every box divides itself by to reach a fan-out width. The figure is one configuration's own rather than the whole tree's, which is what makes it a per-unit cost at all: `peak_rss` maxes `ru_maxrss` over children instead of summing it, `run_m1.build_tables` hands every child `threads=1`, and the `run_m1` parent beside them holds only heads — measured at 0.04 GB against a child's several — so the widest process in that tree is one single-threaded child holding one configuration however wide the fan-out around it is. Seeded off the tree issue #168 landed on — the trace memo declining the probes' cascade windows, over the three memo-side levers of issue #105 (issues #165–#167: the trace memo seated under a packed key, the prospect memo holding only its fallbacks, the candidate memo's lists seated once) — and measured directly rather than read off the cycle-timings journal: a solo `build-tables` child on the whole-alphabet dump peaks at 2.98 GB on the default configuration, measured on that configuration alone because the seed before it found the six acceptance configurations peaking within one percent of each other. The headroom past that is deliberate rather than the tightest divisor the measurement allows, since erring low is what puts a box into swap and erring high only costs a configuration of parallelism — a width that overshot by one was observed paging, at roughly sixty percent longer per configuration for a total wall worse than the next width down — and it is a wider margin than a seed measured on all six configurations would take, because on both fleet boxes in `doc/fleet.md` any divisor at or under this one already answers the whole acceptance set, so a tighter figure would buy no width there while spending the margin one measurement leaves. The peak is still the trace memo's bucket table, which sits under its power-of-two doubling with a third of its capacity free at the whole alphabet — `--cache-census` reads the len beside the cap — so the next letter batches fill the table before they double it. Re-measure it whenever the crate's per-configuration levers land, by timing one child on its own the way `rebuild/tools/kernel_all_configs.py` does and reading `--cache-census` for where the memo sits; it is a reading to keep current, never a contract.
