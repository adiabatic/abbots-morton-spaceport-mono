...
def _composed_walk(rules, unit, context):
    ...
    for index, claims in found.items():
        ...
        if len(judged) == 2 and {claim.kind for claim in judged} == {"gain", "joined"}:
            unjudged = [claim for claim in judged if claim.kind == "joined"] + unjudged
            judged = [claim for claim in judged if claim.kind == "gain"]
        if len(judged) > 1:
            return None
        if not judged:
            if len(unjudged) == 1:
        ...
        if unjudged and judged[0].kind in ("entry", "gain"):
            if len(unjudged) > 1:
                return None
