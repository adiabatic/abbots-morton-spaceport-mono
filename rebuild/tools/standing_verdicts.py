...
def _composed_walk(rules, unit, context):
    ...
    behind_retarget = {
        ...
        and events[index + 1].kind in ("joined", "redrawn", "retarget")
    }
    behind_join = {
        index + 1
    ...
    while index < len(before_names):
        ...
        if event.kind == "slide":
            ...
            if trailing is not None and trailing.kind == "retarget":
                index += 1
            else:
