...
A candidate whose own contract fails is simply not an event and its ink is judged as ordinary span ink, so adding a rule to this file can never un-explain a window; two rules claiming one position, or a join-retargeted or join-created event whose follower position is itself claimed, is ambiguous and refuses. The sanctioned shares of a position are the four chains above — a created join behind an entry contraction is a claim on the letter's seam beside a claim on its entry, and a created join behind a retarget on that retarget's follower, or a retarget or an exit-extension drop behind a created join on that join's follower, is a claim on the letter's outgoing seam beside a claim on its incoming one, none of them two claims on the same thing — and a created join whose pivot moved its origin is not an event at all without a contraction to chain behind. An extension's follower is the named after cell, not a pixel-identity translation, so a named redraw (·May losing the entry the old font stacked on the same seam, ·I's smaller loop after ·Tea) is the rule's own subject and does not block composition.
...
def _composed_walk(rules, unit, context):
    ...
    behind_join = {
        ...
        and events[index + 1].kind in ("retarget", "extension")
    }
    if any(
